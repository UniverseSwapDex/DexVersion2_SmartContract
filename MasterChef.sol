...
// IDAOToken with Governance.
contract IDAOToken is ERC20('UniverseSwap stIDAO', 'stIDAO'), Ownable  {
    uint256 constant private _maxTotalSupply = 200000000e18; // 50,000,000 max supply
...
contract Sugar is ERC20('Sugar', 'SUGAR'), Ownable {
    ...
    // Safe idao transfer function, just in case if rounding error causes pool to not have enough idaos.
    function safeGlideTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 glideBal = idao.balanceOf(address(this));
        if (_amount > glideBal) {
            idao.transfer(_to, glideBal);
        } else {
            idao.transfer(_to, _amount);
        }
    }
}

// MasterChef is the master of idao. He can make idao and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once idao is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of IDAOs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. IDAOs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that IDAOs distribution occurs.
        uint256 accGlidePerShare; // Accumulated IDAOs per share, times 1e18. See below.
        uint256 lpSupply;
    }

    // The IDAO TOKEN!
    IDAOToken public immutable idao;
    // IDAO TOKEN transfer owner
    ...
    // IDAO tokens created per block.
    uint256 public glidePerBlock;
    // IDAO max rate (max perBlock setup)
    ...
    // get idao token reward per block
    ...
    // Update the given pool's IDAO allocation point. Can only be called by the owner.
    ...
    // View function to see pending idaos on frontend.
    function pendingGlide(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            if (idao.totalSupply().add(glidePerBlockCalculated) < idao.getMaxTotalSupply()) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 glidePerBlockCalculated = getGlideReward(pool.lastRewardBlock);
        uint256 glideReward = glidePerBlockCalculated.mul(pool.allocPoint).div(totalAllocPoint);
        if (idao.totalSupply().add(glideReward) > idao.getMaxTotalSupply()) {
            glideReward = (idao.getMaxTotalSupply()).sub(idao.totalSupply());
        }
        // add this check how this function not be fall on require on mint
        if (glideReward != 0) {
            uint256 glideRewardSugar = glideReward.mul(650).div(1000); //65% go to sugar 
            ...
            idao.mint(address(sugar), glideRewardSugar);
            idao.mint(devaddr, glideRewardDev); 
            idao.mint(treasuryaddr, glideRewardTreasury);
    ...
    // Deposit LP tokens to MasterChef for idao allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require (_pid != 0, "MasterChef: deposit IDAO by staking");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        require (_pid != 0, "MasterChef: withdraw IDAO by unstaking");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake IDAO tokens to MasterChef
    ...
    // Withdraw IDAO tokens from STAKING.
    ...
    // Update idao transfer owner. Can only be called by existing idaoTransferOwner
    function setGlideTransferOwner(address _glideTransferOwner) external {
        require(msg.sender == glideTransferOwner, "MasterChef: sender is not idao transfer owner");
        glideTransferOwner = _glideTransferOwner;
    ...
    function transferGlideOwnership(address _newOwner) external {
        require(msg.sender == glideTransferOwner, "MasterChef: sender is not idao transfer owner");
        idao.transferOwnership(_newOwner);

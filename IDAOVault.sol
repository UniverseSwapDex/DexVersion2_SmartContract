...
// File: contracts/Vault/GlideVault.sol
...
contract IDAOVault is Ownable, Pausable {
    ...
    IERC20 public immutable token; // IDAO token
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.glideAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     */
    function calculateTotalPendingGlideRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGlide(0, address(this));
        amount = amount.add(available());

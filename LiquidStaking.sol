...
// File: contracts/LiquidStaking/stIDAOToken.sol
...
contract stIDAOToken is ERC20("Staked IDAO", "stIDAO"), Ownable {
...
contract LiquidStaking is ILiquidStaking, Ownable, ReentrancyGuard {
    ...
    stIDAOToken public immutable stidao;
    ...
    function setReceivePayloadAddress(string calldata _receivePayloadAddress)
        ...
        onlyOwner
    {
        UniverseSwapErrors._require(
            bytes(_receivePayloadAddress).length == 34,
            UniverseSwapErrors.ELASTOS_MAINNET_ADDRESS_LENGTH
        );
    ...
    function updateEpoch(uint256 _exchangeRate) external override onlyOwner {
        UniverseSwapErrors._require(
            ...
            UniverseSwapErrors.EXCHANGE_RATE_MUST_BE_GREATER_OR_EQUAL_PREVIOUS
        );

        UniverseSwapErrors._require(
            _exchangeRate <= exchangeRate.add(_EXCHANGE_RATE_UPPER_LIMIT),
            UniverseSwapErrors.EXCHANGE_RATE_UPPER_LIMIT
        );

        UniverseSwapErrors._require(!onHold, UniverseSwapErrors.STATUS_CANNOT_BE_ONHOLD);
    ...
    function enableWithdraw() external override onlyOwner {
        UniverseSwapErrors._require(onHold, UniverseSwapErrors.STATUS_MUST_BE_ONHOLD);

        UniverseSwapErrors._require(
            prevEpochRequestTotal >= totalELAWithdrawRequested,
            UniverseSwapErrors.UPDATE_EPOCH_NOT_ENOUGH_ELA
        );
    ...
    function deposit() external payable override nonReentrant {
        UniverseSwapErrors._require(
            ...
            UniverseSwapErrors.RECEIVE_PAYLOAD_ADDRESS_ZERO
        );
        ...
        uint256 amountOut = (receivePayloadAmount.mul(_EXCHANGE_RATE_DIVIDER))
            .div(exchangeRate);
        stidao.mint(msg.sender, amountOut);
    ...
    function requestWithdraw(uint256 _stELAAmount) external override nonReentrant {
        UniverseSwapErrors._require(
            _stELAAmount <= stidao.balanceOf(msg.sender),
            UniverseSwapErrors.REQUEST_WITHDRAW_NOT_ENOUGH_AMOUNT
        );
        ...
        stidao.burn(msg.sender, _stELAAmount);
    ...
    function withdraw(uint256 _elaAmount) external override nonReentrant {
        ...
        UniverseSwapErrors._require(
            ...
            UniverseSwapErrors.WITHDRAW_NOT_ENOUGH_AMOUNT
        );
        withdrawReady[msg.sender].elaAmount = withdrawReady[msg.sender]
            .elaAmount
            .sub(_elaAmount);
        ...
        UniverseSwapErrors._require(
            successTransfer,
           UniverseSwapErrors.WITHDRAW_TRANSFER_NOT_SUCCESS
        );
        ...
        override
    {
        UniverseSwapErrors._require(
            ...
            UniverseSwapErrors.SET_STELA_TRANSFER_OWNER
        );
    ...
    function transferstELAOwnership(address _newOwner) external override {
        UniverseSwapErrors._require(
            msg.sender == stELATransferOwner,
            UniverseSwapErrors.TRANSFER_STELA_OWNERSHIP
        );
        stidao.transferOwnership(_newOwner);
